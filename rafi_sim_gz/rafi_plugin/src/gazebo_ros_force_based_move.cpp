...
namespace gazebo 
{
      ...
      sdf::ElementPtr sdf) 
  {
   ...
   // Asegúrate de que el puntero del modelo (parent) no es nulo.
    if (!parent) {
        ROS_ERROR("GazeboRosForceBasedMove: parent model pointer is NULL");
        return;
    }

    parent_ = parent;

    // Asegúrate de que el modelo tiene enlaces disponibles.
    if (parent_->GetLinks().empty()) {
        ROS_ERROR("GazeboRosForceBasedMove: parent model has no links available");
        return;
    }
    ...
    this->link_ = parent_->GetLink(robot_base_frame_);
    int retry_count = 0;
    while (!this->link_ && retry_count < MAX_RETRY) {
        ROS_WARN_STREAM("Link " << robot_base_frame_ << " not found, retrying...");
        gazebo::common::Time::MSleep(1000);  // Espera 100 ms antes de reintentar
        this->link_ = parent_->GetLink(robot_base_frame_);
        retry_count++;
    }

    if (!this->link_) {
        ROS_ERROR_STREAM("Failed to find link: " << robot_base_frame_ << " after retries");
        return;
    }
